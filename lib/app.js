...
function preloadScript(db) {
  ...
  // Runs in <head>, before <body> exists, and sets the theme attributes on
  // <html> (documentElement) so the correct theme is applied before first paint.
  return (
    '<script>(function(){try{' +
...
function renderPage(file) {
  ...
  // content is painted (prevents the gold-theme flash on refresh).
  return html.replace(/<head>/i, '<head>' + preloadScript(db));
}
