...
async function renderPage(file) {
  ...
  // content is painted (prevents the theme flash on refresh).
...
Object.keys(PAGES).forEach((url) => {
  app.get(url, async (req, res) => {
    try {
      ...
      res.type('html').send(await renderPage(PAGES[url]));
    } catch (e) {
...
app.get('/api/status', async (req, res) => {
  ...
  const blobConnected = mode === 'blob' ? await store.probeBlob() : false;
  res.json({ mode, blobConnected });
});
