...
const THEMES = ['gold', 'rose', 'emerald', 'midnight', 'ivory', 'royal', 'dark', 'bw', 'blackemerald', 'silveremerald', 'custom'];
...
const DEFAULT_DB = {
  settings: {
    ...
    shareImage: '',
    theme: 'gold',
    /* Two-colour theme: used when theme === 'custom' (see public/js/theme-colors.js) */
    customColors: { bg: '#ffffff', accent: '#c9a227' },
    currency: '$',
...
function normalize(parsed) {
  ...
  return {
    ...
    products: (Array.isArray(p.products) ? p.products : deepCopy(DEFAULT_DB.products)).map(normalizeProduct)
  };
}

/* Every product always carries a `variants` array (the ml sizes and their
   prices) so the client can render the size picker without null checks.
   Empty array = a single-size product priced by `price`. */
