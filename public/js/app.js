...
const Alwazir = (() => {
  ...
  /* Shrink the header brand font just enough that the FULL name always fits
     on one line — on wide laptops and narrow phones alike. (CSS clamps the
     size by viewport; this finishes the job for long brand names.) */
  function fitHeaderBrands() {
    document.querySelectorAll('.site-header .brand-name, .admin-header .brand-name').forEach((el) => {
      el.style.fontSize = ''; // reset so it can grow back on wider screens
      const base = parseFloat(window.getComputedStyle(el).fontSize) || 16;
      let size = base;
      const min = 11; // px floor — never shrink smaller than this
      let guard = 60; // safety cap on loop iterations
